...
type Entry struct {
	...
	ReviewTags          []ReviewTag            `json:"review_tags"`
}

func (e *Entry) haversineDistance(lat, lon float64) float64 {
	return haversineMeters(e.Latitude, e.Longitude, lat, lon)
}
...
func (e *Entry) IsWebsiteValidForEmail() bool {
	...
	if parsedURL, err := url.Parse(e.WebSite); err == nil {
	...
	// Fallback: raw string check if URL parsing fails.
	rawWebsite := strings.ToLower(e.WebSite)
	for _, blocked := range blockedHosts {
		if strings.Contains(rawWebsite, blocked) {
			return false
		}
	}
...
// SortAndCapReviews merges UserReviews and UserReviewsExtended, sorts them
// newest-first, caps the result at max, and stores it back in UserReviews.
// UserReviewsExtended is cleared afterwards.
func (e *Entry) SortAndCapReviews(max int) {
	all := append(e.UserReviews, e.UserReviewsExtended...)

	parseWhen := func(when string) time.Time {
		var y, m, d int
		if _, err := fmt.Sscanf(when, "%d-%d-%d", &y, &m, &d); err != nil {
			return time.Time{}
		}
		return time.Date(y, time.Month(m), d, 0, 0, 0, 0, time.UTC)
	}

	slices.SortStableFunc(all, func(a, b Review) int {
		ta, tb := parseWhen(a.When), parseWhen(b.When)
		if ta.After(tb) {
			return -1
		}
		if ta.Before(tb) {
...
func extractReviews(data []byte) []Review {
	...
	jsonStr := string(data)
	prefix := ")]}'\n"
	if len(jsonStr) >= len(prefix) && jsonStr[:len(prefix)] == prefix {
		jsonStr = jsonStr[len(prefix):]
	} else if len(jsonStr) >= 4 && jsonStr[0:4] == `)]}'` {
		jsonStr = jsonStr[4:]
	}
	...
	if err := json.Unmarshal(data, &jd); err != nil {
		log.Printf("Error unmarshalling RPC JSON: %v (data len: %d)", err, len(data))
		return nil
	}
...
func EntryFromJSON(raw []byte, reviewCountOnly ...bool) (entry Entry, err error) {
	...
	entry.WebSite = normalizeGoogleURL(getNthElementAndCast[string](darray, 7, 0))
