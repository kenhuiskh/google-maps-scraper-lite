...
func newJobViewWithQueueState(job gmaps.Job, hasActiveJob bool) jobView {
	...
	view := jobView{
		...
		WatchdogTimeouts:    job.ExecutionStats.WatchdogTimeouts,
		BotBlockEvents:      job.ExecutionStats.BotBlockEvents,
		NavigationCDPErrors: job.ExecutionStats.NavigationCDPErrors,
		PageCrashEvents:     job.ExecutionStats.PageCrashEvents,
		StallRestarts:       job.ExecutionStats.StallRestarts,
	}
...
func newAnalyticsView(jobs []gmaps.Job) analyticsView {
	...
	for _, job := range jobs {
		...
		view.WatchdogTimeouts += job.ExecutionStats.WatchdogTimeouts
		view.BotBlockEvents += job.ExecutionStats.BotBlockEvents
		view.NavigationCDPErrors += job.ExecutionStats.NavigationCDPErrors
		view.PageCrashEvents += job.ExecutionStats.PageCrashEvents
		view.StallRestarts += job.ExecutionStats.StallRestarts
	}
	return view
}
